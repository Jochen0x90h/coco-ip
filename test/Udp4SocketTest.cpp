...
int main() {
#endif
    debug::out << "Udp4SocketTest\n";
    debug::out << "Local port " << dec(localPort) << '\n';
    debug::out << "Remote port " << dec(remotePort) << '\n';

    drivers.socket.open(ip::v4::PROTOCOL_ID, localPort);

    drivers.buffer1.header<ip::v4::Endpoint>() = {.port = remotePort, .address = *ip::v4::Address::fromString("127.0.0.1")};
